...
class ABB:
    ...
    #--------------------------Ejercicio 3-----------------------------------#
    def watchFatherBrother(self,raiz,item,father=None,brother=None):
        if raiz!=None:
            if item==self.getRaiz().getValue():
                print("\nEs la raiz, no tiene Padre ni Hermano")
            elif item==raiz.getValue():
                if brother==None:
                    brother=("[No tiene]")
                else:
                    brother=brother.getValue()
                print("\n\nPadre:{}    Hijo:{}    Hermano:{}".format(father.getValue(),raiz.getValue(),brother))
            elif item<raiz.getValue():
                return self.watchFatherBrother(raiz.getLeft(),item,raiz,raiz.getRight())
            else:
                return self.watchFatherBrother(raiz.getRight(),item,raiz,raiz.getLeft())
        else: 
            print("\nError No existe el Nodo")

    def cantNodes(self,raiz):
        if raiz==None:
            return 0
        else:
            return self.cantNodes(raiz.getLeft())+self.cantNodes(raiz.getRight())+1
    
    def watchSuccessors(self,item):
        raiz=self.search(item)
        print("\nSucesores de [%d] modo: InOrden"%(item))
        self.inOrden(raiz)
        print("\n")
...
if __name__=="__main__":
    ...
    a.watchFatherBrother(raiz,3)

    cant=a.cantNodes(raiz)
    print("\nLa cantidad de Nodos es:[%d]"%(cant))

    altura=a.maxHeight()
    print("\nLa Altura es:[%d]"%(altura))

    a.watchSuccessors(12)
